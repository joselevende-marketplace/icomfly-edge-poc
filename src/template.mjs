...
export function renderProductPage({ store, product, products, bakedAt }) {
  ...
  const reviewsScript = [
    ...
    '  function setTop(avg,count){var c=document.querySelector(".pp-rcount");if(c){c.textContent=count+" reseña"+(count===1?"":"s");}var st=document.querySelector(".pp-stars");if(st){var f=Math.max(0,Math.min(5,Math.round(Number(avg)||0)));st.innerHTML="★".repeat(f)+`<span class="soff">`+"★".repeat(5-f)+`</span>`;st.setAttribute("aria-label",f+" de 5");}}',
